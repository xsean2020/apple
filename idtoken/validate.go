...
func (p *Payload) Identify() string {
	return p.Claims["sub"].(string)
}
...
func NewValidator() (*Validator, error) {
	return &Validator{client: newCachingClient(http.DefaultClient)}, nil
}
...
func (v *Validator) validateRS256(ctx context.Context, keyID string, hashedContent []byte, sig []byte) error {
	certResp, err := v.client.getCert(ctx, appleSACertsURL)
	if err != nil {
		return err
	}
